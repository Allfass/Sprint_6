...
class OrderPage(Page):
    ...
    def click_further_order_button(self):
        super().click_on_element_with_javascript(self.order_page_locators.further_button)

    @allure.step('Ждём загрузки страницы')
    def wait_for_load_second_order_page(self):
        super().wait_for_load_page(self.order_page_locators.date_input)

    @allure.step('Вводим имя')
    def input_name(self, value):
        super().input_value(self.order_page_locators.name_input, value)
        
    @allure.step('Вводим фамилию')
    def input_surname(self, value):
        super().input_value(self.order_page_locators.surname_input, value)
        
    @allure.step('Вводим адрес')
    def input_address(self, value):
        super().input_value(self.order_page_locators.address_input, value)
        
    @allure.step('Вводим телефон')
    def input_phone(self, value):
        super().input_value(self.order_page_locators.phone_input, value)
    
    @allure.step('Кликаем на поле с датой')
    def click_date_input_button(self):
        super().click_on_element_with_javascript(self.order_page_locators.date_input)
    ...
    def click_rental_period_button(self):
        super().click_on_element(self.order_page_locators.rental_period_input)

    @allure.step('Кликаем на аренда самоката на один день')
    def click_one_day_rental_period_button(self):
        super().click_on_element_with_javascript(self.order_page_locators.rental_period_dropdown_day)

    @allure.step('Кликаем на аренда самоката на два дня')
    def click_two_days_rental_period_button(self):
        super().click_on_element_with_javascript(self.order_page_locators.rental_period_dropdown_two_days)

    @allure.step('Кликаем на аренда самоката на три дня')
    def click_three_day_rental_period_button(self):
        super().click_on_element_with_javascript(self.order_page_locators.rental_period_dropdown_three_day)

    @allure.step('Выбираем чекбокс "черный жемчуг"')
    def click_black_pearls_color_button(self):
        super().click_on_element_with_javascript(self.order_page_locators.checkbox_black_pearls)

    @allure.step('Выбираем чекбокс "серая безысходность"')
    def click_gray_despair_color_button(self):
        super().click_on_element_with_javascript(self.order_page_locators.checkbox_gray_despair)

    @allure.step('Кликаем кнопку заказа')
    def click_order_button(self):
        super().click_on_element_with_javascript(self.order_page_locators.order_button)

    @allure.step('Кликаем подтвердить заказ')
    def click_accept_order_button(self):
        super().click_on_element_with_javascript(self.order_page_locators.accept_order_button)

    @allure.step('Кликаем отменить заказ')
    def click_decline_order_button(self):
        super().click_on_element_with_javascript(self.order_page_locators.decline_order_button)

    @allure.step('Проверяем отображения поля статуса заказа')
    def check_compete_order_message(self):
        return super().check_page_load(self.order_page_locators.compete_order_message)
